...
const crossVector = (left: Vector, right: Vector) =>
...
const getClosestPointOnSegment = (
  ...
  if (lengthSquared <= POSITION_EPSILON) {
    return {
      distance: getVectorMagnitude(subtractVector(point, start)),
      point: start,
      t: 0,
    }
  }
  ...
  const t = clampUnitInterval(
    dotVector(subtractVector(point, start), segment) / lengthSquared,
  )
  ...
  return {
    distance: getVectorMagnitude(subtractVector(point, projectedPoint)),
...
const getSegmentIntersection = (
  ...
  const leftDelta = subtractVector(leftEnd, leftStart)
  const rightDelta = subtractVector(rightEnd, rightStart)
  const determinant = crossVector(leftDelta, rightDelta)

  if (Math.abs(determinant) <= POSITION_EPSILON) {
    return null
  }

  const offset = subtractVector(rightStart, leftStart)
  const leftT = crossVector(offset, rightDelta) / determinant
  const rightT = crossVector(offset, leftDelta) / determinant

  if (
    leftT < -POSITION_EPSILON ||
    leftT > 1 + POSITION_EPSILON ||
    rightT < -POSITION_EPSILON ||
    rightT > 1 + POSITION_EPSILON
  ) {
    return null
  }
  ...
  return {
    leftPoint: {
      x: leftStart.x + leftDelta.x * leftT,
      y: leftStart.y + leftDelta.y * leftT,
    },
    leftT: clampUnitInterval(leftT),
    rightPoint: {
      x: rightStart.x + rightDelta.x * rightT,
      y: rightStart.y + rightDelta.y * rightT,
    },
    rightT: clampUnitInterval(rightT),
  }
}

const getSegmentSegmentProximity = (
  leftSegment: SegmentObstacle,
  rightSegment: SegmentObstacle,
) => {
  const intersection = getSegmentIntersection(
    leftSegment.startNode,
    leftSegment.endNode,
    rightSegment.startNode,
    rightSegment.endNode,
  )

  if (intersection) {
    return {
      distance: 0,
      leftPoint: intersection.leftPoint,
      leftT: intersection.leftT,
      rightPoint: intersection.rightPoint,
      rightT: intersection.rightT,
    }
  }

  const leftStartToRight = getClosestPointOnSegment(
    leftSegment.startNode,
    rightSegment.startNode,
    rightSegment.endNode,
  )
  const leftEndToRight = getClosestPointOnSegment(
    leftSegment.endNode,
    rightSegment.startNode,
    rightSegment.endNode,
  )
  const rightStartToLeft = getClosestPointOnSegment(
    rightSegment.startNode,
    leftSegment.startNode,
    leftSegment.endNode,
  )
  const rightEndToLeft = getClosestPointOnSegment(
    rightSegment.endNode,
    leftSegment.startNode,
    leftSegment.endNode,
  )
  ...
  for (let passIndex = 0; passIndex < passCount; passIndex += 1) {
    ...
    for (let leftIndex = 0; leftIndex < segments.length; leftIndex += 1) {
      ...
      for (
        ...
        if (leftSegment.rootConnectionName === rightSegment.rootConnectionName) {
          continue
        }
        if (leftSegment.z !== rightSegment.z) {
          continue
        }

        const leftMinX = Math.min(leftSegment.startNode.x, leftSegment.endNode.x)
        const leftMaxX = Math.max(leftSegment.startNode.x, leftSegment.endNode.x)
        const leftMinY = Math.min(leftSegment.startNode.y, leftSegment.endNode.y)
        const leftMaxY = Math.max(leftSegment.startNode.y, leftSegment.endNode.y)
        const rightMinX = Math.min(
          rightSegment.startNode.x,
          rightSegment.endNode.x,
        )
        ...
        const rightMinY = Math.min(
          rightSegment.startNode.y,
          rightSegment.endNode.y,
        )
        ...
        if (
          leftMaxX + TARGET_CLEARANCE < rightMinX ||
          rightMaxX + TARGET_CLEARANCE < leftMinX ||
          leftMaxY + TARGET_CLEARANCE < rightMinY ||
          rightMaxY + TARGET_CLEARANCE < leftMinY
        ) {
          continue
        }

        const proximity = getSegmentSegmentProximity(leftSegment, rightSegment)
        if (!proximity) continue
        const penetration = TARGET_CLEARANCE - proximity.distance
        if (penetration <= 0) continue

        const separation = subtractVector(
          proximity.leftPoint,
          proximity.rightPoint,
        )
        const fallbackSeed = passIndex * 1_009 + leftIndex * 97 + rightIndex * 13
        const direction =
          proximity.distance > POSITION_EPSILON
            ? scaleVector(separation, 1 / proximity.distance)
            : getPerpendicularVector(
                subtractVector(leftSegment.endNode, leftSegment.startNode),
                fallbackSeed,
              )
        ...
        const correctionX = direction.x * magnitude
        const correctionY = direction.y * magnitude
  ...
  for (let stepIndex = 0; stepIndex < totalSteps; stepIndex += 1) {
    ...
    for (let leftIndex = 0; leftIndex < segments.length; leftIndex += 1) {
      ...
      for (
        ...
        if (
          leftMaxX + CLEARANCE_FALLOFF_DISTANCE < rightMinX ||
          rightMaxX + CLEARANCE_FALLOFF_DISTANCE < leftMinX ||
          leftMaxY + CLEARANCE_FALLOFF_DISTANCE < rightMinY ||
          rightMaxY + CLEARANCE_FALLOFF_DISTANCE < leftMinY
        ) {
          continue
        }
        ...
        const forceX = direction.x * magnitude
        const forceY = direction.y * magnitude
